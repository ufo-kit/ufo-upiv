                                 ...
                                 GError **error)
{
    ...
    priv->kernel = ufo_resources_get_kernel (resources, "hough.cl", "likelihood", NULL, error);
    ...
    priv->mask_mem = clCreateBuffer (priv->context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, priv->masksize * priv->masksize * sizeof (*mask), mask, &err);
