...
/**
 * SECTION:ufo-concatenate-result-task
 * @Short_description: Write TIFF files
 * @Title: concatenate_result
 *
 */
...
increase_buffer_size(UfoConcatenateResultTaskPrivate *priv)
{
    ...
    priv->metadata = g_realloc(priv->metadata,
                               sizeof (PivFileMetadata) * priv->img_alloc_size);
                                     ...
                                     UfoRequisition *requisition)
{
    ...
    if (priv->idx + nb_coord > priv->alloc_size) {
        ...
        priv->data[piv_file_idx] = g_realloc (priv->data[piv_file_idx],
                                              sizeof (UfoRingCoordinate) *
                                              priv->alloc_size + sizeof (float));
    }
                                      ...
                                      UfoRequisition *requisition)
{
    ...
    UfoRequisition new_req = {
        .dims[0] = 1 + (unsigned) rings->nb_elt * sizeof (UfoRingCoordinate) /
            sizeof (float),
        .n_dims = 1,
    };
    ufo_buffer_resize (output, &new_req);
    float *res = ufo_buffer_get_host_array (output, NULL);
    memcpy (res, rings, (unsigned) (rings->nb_elt) * sizeof (UfoRingCoordinate) +
            sizeof (float));
                              ...
                              GParamSpec *pspec)
{
    ...
    switch (property_id) {
        ...
        case PROP_RING_COUNT:
            ...
            priv->metadata = g_malloc (sizeof (PivFileMetadata) *
                                       priv->ring_count);
