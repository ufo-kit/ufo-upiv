...
/**
 * SECTION:ufo-filter-particle-task
 * @Short_description: Write TIFF files
 * @Title: filter_particle
 *
 */
...
void part_cen_radius(float *img, unsigned *bin, unsigned nc, unsigned nr,
       ...
       double threshold)
{
  ...
  *pcount=ncc;
   ...
   * one */
  for (k=0;k<=pc;k++) dupindex[k]= dupy[ dupindex[k] ];
                                  ...
                                  UfoRequisition *requisition)
{
    ...
    pfind (input, priv->img, (unsigned) req.dims[0], (unsigned) req.dims[1],
           &pcount, threshold);
    ...
    UfoRequisition out_req = {
        .n_dims = 1,
        .dims[0] = 2 + pcount * sizeof (UfoRingCoordinate) / sizeof (float),
    };
    ufo_buffer_resize (output, &out_req);
    float *out = ufo_buffer_get_host_array(output, NULL);
    UfoRingCoordinate *res = (UfoRingCoordinate *) (out + 2);
    *out = (float) pcount;
    ...
    g_message("%f %f %f", out[0], out[1], out[2]);
    return TRUE;
}
